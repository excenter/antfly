...
pub fn build(b: *std.Build) void {
    ...
    const lib_data_runtime_default_filters = [_][]const u8{
        ...
        "remote metadata source accepts a fresh snapshot after metadata process restart",
        "remote metadata source rejects a superseded normal snapshot fetch",
        "remote metadata source accepts a newer snapshot from an earlier fetch",
        "remote metadata source rejects a later fetch from a lagging replica",
        "remote metadata source legacy snapshot does not pin the cache",
        "remote metadata source legacy follower does not regress the cache",
        "remote metadata source rejects a normal snapshot across invalidation",
        "remote metadata source fenced snapshot blocks an older normal fetch",
    ...
    const api_table_writes_docid_tests = b.addTest(.{
        ...
        .filters = selectTestFilters(b, &.{
            ...
            "write cache invalidation retires leased entry until release",
            "retired group rejects writer cache resurrection until reactivated",
    ...
    const api_table_reads_docid_tests = b.addTest(.{
        ...
        .filters = &.{
            ...
            "provisioned read cache rechecks group admission across open and lease publication",
